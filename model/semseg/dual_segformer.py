...
from model.backbone.dual_segformer_backbone import mit_b0, mit_b1, mit_b2, mit_b3, mit_b4, mit_b5
...
class dual_SegFormer(nn.Module):
    def __init__(self, num_classes = 2, phi = 'b0', pretrained = False):
        super(dual_SegFormer, self).__init__()
        self.in_channels = {
    ...
    def forward(self, inputs1, inputs2, need_fp=False):
        H, W = inputs1.size(2), inputs1.size(3)
        
        x = self.backbone.forward(inputs1, inputs2)
...
if __name__ == "__main__":
    ...
    model = dual_SegFormer(pretrained=False).cuda()
    # model.get_param_groupsv()
    inputs1 = torch.rand(2, 3, 512, 512).cuda()
    inputs2 = torch.rand(2, 512, 512).cuda()
