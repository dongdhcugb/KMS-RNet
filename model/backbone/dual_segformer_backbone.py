...
from ..dual_segnet_utils import FeatureFusionModule as FFM
from ..dual_segnet_utils import FeatureRectifyModule as FRM
import math
import time
...
class RGBXTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, in_chans_dem=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.extra_patch_embed1 = OverlapPatchEmbed(img_size=img_size, patch_size=7, stride=4, in_chans=in_chans_dem,
                                              embed_dim=embed_dims[0])
        ...
        self.block2 = nn.ModuleList([Block(
            ...
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur], norm_layer=norm_layer,  # drop_path=dpr[cur]
        ...
        self.extra_block2 = nn.ModuleList([Block(
            dim=embed_dims[1], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + 1], norm_layer=norm_layer,  # drop_path=dpr[cur + 1]
            sr_ratio=sr_ratios[1])
            for i in range(depths[1])])
        ...
        self.FRMs = nn.ModuleList([
                    FRM(dim=embed_dims[0], reduction=1),  # reduction=1
